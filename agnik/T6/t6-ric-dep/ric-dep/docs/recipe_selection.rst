...
A **recipe** is a YAML configuration file that defines: - Component
versions (image tags) - Namespaces - Resource limits - Configuration
parameters - Image registries - External service IPs
...
Required fields in every recipe: - ``common.releasePrefix`` -
``extsvcplt.ricip`` (auto-updated) - ``extsvcplt.auxip`` (auto-updated)
...
**Solution:** - Compare with example recipes - Ensure all required
sections present - Check indentation matches examples

...
- See [Platform Requirements](PLATFORM_REQUIREMENTS.md) for detailed network/firewall configuration
...
- Base packages (curl, git, python3, etc.)

### Option 2: Manual

See [Platform Requirements](PLATFORM_REQUIREMENTS.md) for manual configuration steps.
...
This installs:
- Helm 3.14.4
- Kubernetes 1.30.14 (containerd, kubeadm, kubelet, kubectl)
- Flannel CNI
...
**Limitations:**
- MicroK8s uses different CNI (not Flannel)
- Some Near-RT RIC components may need adjustment
- Not tested with current deployment scripts
...
See [Recipe Selection Guide](RECIPE_SELECTION.md) for details.
...
This automatically:
- Packages ric-common chart
- Starts local Helm repository
- Updates recipe IP addresses
- Deploys all Near-RT RIC components
- Waits for AppMgr
- Restarts RTMgr
...
1. **Configure xApps**: See [xApp Deployment Guide](installation-xapps.rst)
